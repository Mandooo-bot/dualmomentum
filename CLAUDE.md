...
입력된 티커를 아래 5가지 카테고리로 자동 분류:

| 카테고리 | 설명 |
|----------|------|
| **인덱스 코어** | 시장 전체를 추종하는 핵심 ETF (예: VOO, QQQ, VTI) |
| **유망 섹터 ETF** | 특정 섹터에 집중된 ETF (예: XLK, SOXX, XLE) |
| **알파 후보** | 현재 모멘텀은 높지 않으나 장기 상승 가능성이 있는 자산 |
| **모멘텀/고베타** | 현재 모멘텀이 높거나 변동성이 큰 자산 |
| **기타** | 위 분류에 해당하지 않는 자산 |
